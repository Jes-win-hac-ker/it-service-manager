...
const SearchReports: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white dark:bg-brand-grey-dark rounded-lg shadow-md p-6">
            ...
            reports.map((report) => (
              <div key={report.id} className="border border-gray-200 dark:border-brand-grey rounded-lg p-4">
                ...
                <div className="flex justify-end items-center gap-2 mt-2">
                  <button onClick={() => openEditModal(report)} className="flex items-center gap-2 text-sm bg-gray-100 text-brand-grey-dark px-3 py-1 rounded-md hover:bg-gray-200 dark:bg-brand-grey dark:text-gray-200 dark:hover:bg-brand-grey-light">
        ...
        {hasMore && !isLoading && (
          <div className="mt-6 text-center">
            <button onClick={loadMore} disabled={isLoadingMore} className="bg-gray-200 dark:bg-brand-grey text-gray-800 dark:text-gray-200 py-2 px-6 rounded-lg hover:bg-gray-300 dark:hover:bg-brand-grey-light disabled:opacity-50">
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="status" value={formData.status} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1">
                    {statusOptions.map(opt => <option key={opt}>{opt}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Customer Name</label>
                  <input name="customer_name" value={formData.customer_name} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Part Name</label>
                  <input name="part_name" value={formData.part_name || ''} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Part ID</label>
                  <input name="part_number" value={formData.part_number || ''} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Invoice Number</label>
                  <input name="invoice_number" value={formData.invoice_number || ''} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Shop Purchased From</label>
                  <input name="shop_name" value={formData.shop_name || ''} onChange={handleInputChange} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Problem Description</label>
                <textarea name="problem_description" value={formData.problem_description} onChange={handleInputChange} rows={3} className="w-full p-2 border border-gray-300 dark:border-brand-grey bg-white dark:bg-brand-grey text-gray-900 dark:text-white rounded-lg mt-1"/>
              </div>
              <div className="flex justify-end gap-4 pt-4">
                <button type="button" onClick={() => setReportToEdit(null)} className="bg-gray-200 dark:bg-brand-grey py-2 px-4 rounded-lg">Cancel</button>
      ...
      {reportToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-brand-grey-dark rounded-lg shadow-xl max-w-sm w-full p-6">
            ...
            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
              ...
              <button type="button" onClick={() => setReportToDelete(null)} className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-brand-grey shadow-sm px-4 py-2 bg-white dark:bg-brand-grey text-base font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-brand-grey-light sm:mt-0 sm:w-auto sm:text-sm">
                Cancel
              </button>
            </div>
          </div>
